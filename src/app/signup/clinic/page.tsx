...
function ClinicForm() {
  ...
  const [form, setForm] = useState({
    name: "", institutionName: "", institutionType: "clinic" as "hospital" | "clinic" | "individual",
    doctorRegNumber: "", specialisation: "", businessAddress: "",
    phone: "", email: "", yearsOfPractice: "", password: "", confirm: "",
  });
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: Record<string, unknown> = {
        ...
        password: form.password, role: "clinic", subscribe,
        institutionName: form.institutionName,
        institutionType: form.institutionType,
        doctorRegNumber: form.doctorRegNumber,
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
      ...
      <p style={{ fontSize: "11px", color: "#C9963E", fontWeight: 600, letterSpacing: "0.16em", textTransform: "uppercase", marginBottom: "10px" }}>
        {step === 1 ? "Clinical Registration" : "Verify account"}
      </p>
      <h1 style={{ fontFamily: "Cormorant Garamond, Georgia, serif", fontSize: "2.2rem", fontWeight: 700, color: "#0B1F3A", marginBottom: "8px", lineHeight: 1.1 }}>
        {step === 1 ? "Clinic / Hospital Registration" : "Enter OTP"}
      </h1>
      <p style={{ fontSize: "14px", color: "#6E6E73", marginBottom: "32px", lineHeight: 1.6 }}>
        {step === 1 ? "Register your clinic, hospital, or individual practice." : `We sent a 6-digit code to ${email}`}
      </p>
      ...
      {step === 1 && (
        <form onSubmit={handleSignup} style={{ display: "flex", flexDirection: "column", gap: "18px" }}>
          <div>
            <label style={lbl}>Doctor / Contact Name <span style={{ color: "#f87171" }}>*</span></label>
          ...
          <div>
            <label style={lbl}>Institution Name <span style={{ color: "#f87171" }}>*</span></label>
            <input value={form.institutionName} onChange={(e) => setForm({ ...form, institutionName: e.target.value })} className="input-premium" placeholder="Hospital / clinic name" required />
          </div>
          <div>
            <label style={lbl}>Institution Type <span style={{ color: "#f87171" }}>*</span></label>
            <select value={form.institutionType} onChange={(e) => setForm({ ...form, institutionType: e.target.value as "hospital" | "clinic" | "individual" })} className="input-premium">
          ...
          <div>
            <label style={lbl}>Doctor Registration Number <span style={{ color: "#f87171" }}>*</span></label>
            <input value={form.doctorRegNumber} onChange={(e) => setForm({ ...form, doctorRegNumber: e.target.value })} className="input-premium" placeholder="Medical council registration number" required />
          </div>
          <div>
            <label style={lbl}>Specialisation <span style={{ color: "#f87171" }}>*</span></label>
            <input value={form.specialisation} onChange={(e) => setForm({ ...form, specialisation: e.target.value })} className="input-premium" placeholder="e.g. Ophthalmology, General Medicine" required />
          </div>
          <div>
            <label style={lbl}>Business Address <span style={{ color: "#f87171" }}>*</span></label>
            <textarea rows={3} value={form.businessAddress} onChange={(e) => setForm({ ...form, businessAddress: e.target.value })} className="input-premium" placeholder="Clinic / hospital address" required />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="number" min="0" value={form.yearsOfPractice} onChange={(e) => setForm({ ...form, yearsOfPractice: e.target.value })} className="input-premium" placeholder="Optional" />
          </div>
          <div>
          ...
          <label style={{ display: "flex", alignItems: "flex-start", gap: "10px", cursor: "pointer" }}>
            ...
            <span style={{ fontSize: "12px", color: "#6E6E73", lineHeight: 1.5 }}>Subscribe to product updates and clinical resources</span>
          </label>
          ...
          <button type="submit" disabled={loading} className="btn-primary btn-gold" style={{ fontSize: "14px", padding: "14px 20px", opacity: loading ? 0.6 : 1 }}>
            {loading ? <span style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "8px" }}><Loader2 size={15} className="animate-spin" />Creating account...</span> : "Create Clinic Account"}
          </button>
        </form>
      )}
